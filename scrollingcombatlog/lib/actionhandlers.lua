...
actionhandlers.parse_action_packet = function(act)
    if not Self then
        ...
        if not Self then return act end
    end

    if not SelfPlayer then
			SelfPlayer = AshitaCore:GetMemoryManager():GetPlayer()
			if not SelfPlayer then return act end
		end

		-- Resolve actor and action
		act.actor = gActionHandlers.ActorParse(act.actor_id)
		act.action = gActionHandlers.SpellParse(act)

		if not act.action or not act.action.name then
			act.action = act.action or {}
			act.action.name = 'hit'
		end

		-- Clean up bad SJIS bytes in actor name (rare, mostly cosmetic)
		if act.actor and act.actor.name then
			act.actor.name = act.actor.name
				:gsub('[- ]', { ['-'] = string.char(0x81, 0x7C), [' '] = string.char(0x81, 0x3F) })
				:gsub('[%z\1-\31]', ''):gsub('%z+', '');
		end

		-- Parse each target and resolve entity name
		-- for _, v in ipairs(act.targets) do
			-- v.target = {};
			-- v.target[1] = gFuncs.GetEntityByServerId(v.server_id);
		-- end

		-- Extract display information from first action (most common use case)
		if act.targets and act.targets[1] and act.targets[1].actions and act.targets[1].actions[1] then
			local first_action = act.targets[1].actions[1];
			act.action.param = first_action.param or 0;
			act.action.name = first_action.simp_name or act.action.name or '?';
		else
			act.action.param = act.action.param or 0;
			act.action.name = act.action.name or '?';
		end
...
for i, v in ipairs(act.targets) do
    ...
    v.target = {};
    local entity = gFuncs.GetEntityByServerId(v.server_id)
		if entity then
			v.target[1] = entity
		else
			gFuncs.Error(string.format('GetEntityByServerId failed for server_id: %08X', v.server_id or 0))
			v.target[1] = { name = '[Unknown]', ServerId = v.server_id or 0, id = v.server_id or 0 }
		end

	if not v.target[1] then
		gFuncs.Error(string.format('Failed to resolve target for server_id: %08X', v.server_id or 0))
	end
        --Handlers.ActorParse(v.server_id);
        if type(v.actions) == 'table' and next(v.actions, next(v.actions)) ~= nil then
            ...
            local tempact = (type(v.actions) == 'table' and v.actions[1]) or nil
			if tempact then
    ...
    for i, v in pairs(act.targets) do
        for n, m in pairs(v.actions) do
            if m.message ~= 0 and res_actmsg[m.message] ~= nil then
				...
				-- Fallback if target is unresolved
				local target_display
				if v.target[1] and v.target[1].name then
					target_display = v.target[1].name
				elseif v.target[1] and v.target[1].id then
					target_display = string.format('[%08X]', v.target[1].id)
				else
				...
				if not act.action or not act.action.name then
					...
					m.simp_name = act.action.name
				end
				if not m.simp_name then
					if not act.action or not act.action.name then
						m.simp_name = 'hit'
					else
						m.simp_name = act.action.name
    ...
    if act.targets and act.targets[1] and act.targets[1].actions and act.targets[1].actions[1] then
        ...
        act.action.param = action.param or 0;
        act.action.name = action.simp_name or act.action.name or '?';
    end
...
actionhandlers.SpellParse = function (act)
    ...
    if spell.data and not spell.name then
		spell.name = spell.data.Name[gProfileSettings.lang.object] or spell.data.Name[1] or '?'
	end
